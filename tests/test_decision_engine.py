...
def test_single_sentiment_signal_scores_below_threshold():
...
def test_pattern_breakout_alone_scores_above_threshold():
...
def test_volume_anomaly_amplifies_existing_directional_score():
    ...
    score, _ = compute_raw_score(signals)
    assert score == pytest.approx(-0.9 * 1.25)
...
def test_decide_buy_when_score_crosses_threshold():
    signals = [{"time": ist(11, 0), "signal_type": "pattern", "value": "breakout", "confidence_score": None}]
...
def test_decide_sell_when_score_below_negative_threshold():
    signals = [
        {"time": ist(11, 0), "signal_type": "sentiment", "value": "negative", "confidence_score": 1.0},
        {"time": ist(11, 1), "signal_type": "volume", "value": "anomaly", "confidence_score": None},
    ]
    decision = decide("RELIANCE", signals, now=ist(11, 0))
...
def test_buy_signal_decays_within_decay_window():
    signals = [{"time": ist(14, 30), "signal_type": "pattern", "value": "breakout", "confidence_score": None}]
    early = decide("RELIANCE", signals, now=ist(14, 5))   # just past decay start, near-full weight
...
def test_buy_blocked_entirely_at_or_after_entry_cutoff():
    signals = [{"time": ist(15, 5), "signal_type": "pattern", "value": "breakout", "confidence_score": None}]
...
def test_buy_blocked_before_market_open():
    signals = [{"time": ist(8, 0), "signal_type": "pattern", "value": "breakout", "confidence_score": None}]
...
def test_sell_is_never_decayed_near_close():
    signals = [
        {"time": ist(15, 10), "signal_type": "sentiment", "value": "negative", "confidence_score": 1.0},
